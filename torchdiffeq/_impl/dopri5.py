...
class Dopri5Solver(AdaptiveStepsizeODESolver):
    '''
    def __init__(
        self, func, y0, t2, h, rtol, atol, first_step=None, safety=0.9, ifactor=10.0, dfactor=0.2, max_num_steps=2**31 - 1,
        **unused_kwargs
    ):
    '''
    def __init__(
    ...
    def before_integrate(self, t):
        #f0 = self.func(t[0].type_as(self.y0[0]), self.y0, self.t2, self.h)
        f0 = self.func(t[0].type_as(self.y0[0]), self.y0)
        if self.first_step is None:
            #first_step = _select_initial_step(self.func, t[0], self.y0,self.t2, self.h, 4, self.rtol[0], self.atol[0], f0=f0).to(t)
        ...
        #self.rk_state = _RungeKuttaState(self.y0, f0, t[0], t[0],self.t2, self.h, first_step, interp_coeff=[self.y0] * 5)
    ...
    def _adaptive_dopri5_step(self, rk_state):
        ...
        #y0, f0, _, t0,t2,h, dt, interp_coeff = rk_state
        ...
        #y1, f1, y1_error, k = _runge_kutta_step(self.func, y0, f0, t0, t2, h, dt, tableau=_DORMAND_PRINCE_SHAMPINE_TABLEAU)
        ...
        #rk_state = _RungeKuttaState(y_next, f_next, t0, t_next,t2, h, dt_next, interp_coeff)
